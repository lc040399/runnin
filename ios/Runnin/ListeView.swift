...
struct ListeView: View {
    ...
    var titel = "Kommende løb"
    var visAntal = true
    var tomTekst = "Ingen løb med de filtre."
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack(alignment: .firstTextBaseline) {
                Text(titel).font(.system(size: 24, weight: .bold)).foregroundColor(ink)
                Spacer()
                if visAntal {
                    HStack(spacing: 3) {
            ...
            if liste.isEmpty {
                VStack(spacing: 8) {
                    ...
                    Text(tomTekst).font(.system(size: 15)).foregroundColor(muted)
                }
...
struct KommerSnartView: View {
    ...
    var body: some View {
        VStack(spacing: 14) {
            ...
            Text("Kommer snart i den native app.").font(.system(size: 15)).foregroundColor(muted)
        }
        .frame(maxWidth: .infinity, maxHeight: .infinity)
